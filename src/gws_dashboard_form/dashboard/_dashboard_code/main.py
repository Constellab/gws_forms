...
SESSIONS_DIR = os.path.join(os.path.abspath(os.path.dirname(__file__)), "saved_sessions")
if not os.path.exists(SESSIONS_DIR):
    os.makedirs(SESSIONS_DIR)

# Create a directory for saving submitted sessions if it doesn't exist
SESSIONS_SUBMITTED_DIR = os.path.join(os.path.abspath(os.path.dirname(__file__)), "submitted_sessions")
...
def show_submitted_sessions(submitted_directory: str):
    ...
    files = [f.split(".json")[0] for f in os.listdir(submitted_directory) if f.endswith(".json")]
    # If there are files, show a selectbox to choose one
    if files:
        selected_file = st.selectbox(label = "Choisissez une session soumise à afficher", options = files, index = None,placeholder="Sélectionnez une session")
        ...
        if selected_file:
            ...
            st.download_button(label="Télécharger les réponses soumises",
                            data=json.dumps(submitted_data, indent=4, ensure_ascii=False).encode('utf-8'),
...
def show_content():
    ...
    with tab_questions:
        ...
        #User name
        name_user = st.text_input(label="Entrez un nom", placeholder="Entrez une réponse", value = session_choice.split("-")[1] if session_choice else "")

        # Load previous answers if available
        saved_answers = load_session(session_name = session_choice, session_directory = SESSIONS_DIR) if session_choice else {}

        # Regrouper les questions par section
        sections = get_questions_by_section_and_subsection(json_questions['questions'])
        ...
        for section, subsections in sections.items():
            ...
            for subsection, questions in subsections.items():
                ...
                for question_data in questions:
                    ...
                    st.markdown(f"#### {question_data['question_head']}: {question_key}")
                    ...
                    if question_data.get('allowed_values') :
                        if question_data.get('multiselect'):
                            ...
                            options=question_data['allowed_values'], default=saved_answer if saved_answer else None, placeholder="Sélectionnez une option ou plusieurs")
                        else :
                    ...
                    if question_data["required"] and (response is None or response == ""):
                        st.write(":red[*Réponse obligatoire]")

                st.markdown("---")

        # Submit button will only be enabled if all required answers are filled
        submit_disabled = not all_required_answered(json_questions['questions'])

        # Save button
        st.write("Si vous souhaitez revenir plus tard compléter vos réponses, cliquez sur 'Enregistrer la session'.")
        col1, col2, col3 = st.columns([1,1,1])
        with col2:
            if st.button("Enregistrer la session", use_container_width=True):
                save_current_session(questions = json_questions['questions'], session_directory = SESSIONS_DIR, name_user=name_user)
        ...
        with col2:
            if st.button("Soumettre", disabled=submit_disabled, type="primary", use_container_width=True):
                save_current_session(questions = json_questions['questions'], session_directory = SESSIONS_SUBMITTED_DIR, name_user = name_user)
                #Delete the file where the session was saved if it's not a new session
                if session_choice :
                    session_path = os.path.join(SESSIONS_DIR, session_choice)
                    # Check if the file exists and delete it
