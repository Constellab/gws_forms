...
div[data-testid="stVerticalBlockBorderWrapper"]:has(div.opaque-container-{id}):not(:has(div.not-opaque-container)) {{
    background-color: var(--background-color);
    width: 100%;
    height: auto;
    overflow: hidden; /* Prevent it from spilling outside */
    z-index: 0; /* Ensure it's beneath other elements */
}}
div[data-testid="stVerticalBlock"]:has(div.opaque-container-{id}):not(:has(div.not-opaque-container)) > div[data-testid="element-container"] {{
    display: none;
}}
...
div[data-testid="stVerticalBlockBorderWrapper"]:has(div.fixed-container-{id}):not(:has(div.not-fixed-container)){{
    position: {mode};
    {position}: {margin};
    z-index: 1; /* Ensure it's just above the background but below interactable elements */
    background-color: transparent;
    width: inherit;
    height: auto; /* Limit height to prevent overflow */
    overflow: hidden; /* Prevent unwanted expansion */
}}
div[data-testid="stVerticalBlock"]:has(div.fixed-container-{id}):not(:has(div.not-fixed-container)) > div[data-testid="element-container"] {{
    display: none;
}}
...
def st_fixed_container(
    *,
    height: int | None = None,
    border: bool | None = None,
    mode: Literal["fixed", "sticky"] = "sticky",
