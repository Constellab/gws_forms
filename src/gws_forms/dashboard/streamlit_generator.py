...
class GenerateFormsDashboard(AppConfig):
    ...
    def get_app_folder_path(self):
        return os.path.join(os.path.abspath(os.path.dirname(__file__)), "_form_dashboard_code")


@task_decorator(
    "StreamlitFormsDashbaordGenerator",
    human_name="Forms dashboard",
    short_description="Task to generate a custom Streamlit dashboard to create form",
    style=TypingStyle.material_icon(
        material_icon_name="question_answer", background_color="#413ebb"
    ),
)
class StreamlitFormsDashbaordGenerator(Task):
    ...
    config_specs: ConfigSpecs = ConfigSpecs(
        {
            "title": StrParam(
                human_name="Title", short_description="Title of the form", optional=False
            ),
