...
def show_content():
    ...
    with tab_creation:
        ...
        session_choice = st.selectbox("Select a previous session to load it.", options=session_list, index=None) if session_list else None
        if session_choice:
        ...
        with col3:
            question_head = st.text_input(label="Question_head", placeholder="Enter a keyword for your question", key = "Question_head")
        question = st.text_input(label="Question", placeholder="Enter the question", key = "Question")
        helper_text = st.text_input("Helper Text", placeholder="Provide some context for the question", key = "Helper_text")
        is_required = st.checkbox("Is this question required?", key = "Is_required")

        response_type = st.selectbox("Response Type", ["long_text","short_text", "option", "numeric"], key = "Response_type")
        if response_type == "option":
            allowed_values = st.text_area("Allowed Values (comma-separated)", placeholder="e.g., Yes, No, Maybe", key = "Allowed_values")
        ...
        if st.button("Submit Question", disabled = submit_disabled, on_click=clear_fields):
            question = {
                ...
                "allowed_values": st.session_state["allowed_values"].split(",") if "allowed_values" in st.session_state else [],
                "multiselect": st.session_state["multi_select"] if "multi_select" in st.session_state else False,
                "min_value": st.session_state["min_value"] if "min_value" in st.session_state else None,
                "max_value": st.session_state["max_value"] if "max_value" in st.session_state else None,
        ...
        if st.session_state.questions:
            ...
            with col2:
                if st.button("Save session", use_container_width=True, key = "save_end"):
                    save_current_session(questions=st.session_state.questions, session_directory=SESSIONS_DIR, name_user=name_user)
            ...
            with col2:
                if st.button("Submit", type="primary", use_container_width=True):
                    save_current_session(questions=st.session_state.questions, session_directory=SESSIONS_SUBMITTED_DIR, name_user=name_user)
                    # Delete the file where the session was saved if it's not a new session
                    if session_choice:
                        session_path = os.path.join(SESSIONS_DIR, session_choice)
                        # Check if the file exists and delete it
